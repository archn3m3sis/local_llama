...
from ..components.dat_update_form import dat_update_form
from ..components.dat_update_table import dat_update_table
from ..states.dat_update_state import DatUpdateState


def dats_with_custom_wrapper() -> rx.Component:
    """DAT updates page with complete form and table functionality."""
    return rx.fragment(
        # Universal background from local_llama.py
        rx.box(id="universal-background"),
        
        # DAT updates page content
        rx.vstack(
            # Massive metallic title matching dashboard style  
            metallic_title("IAMS - DAT File Updates"),
            
            # Content area
            rx.vstack(
                rx.text(
                    "Record DAT file updates and track antivirus definition deployments", 
                    color="gray.400", 
                    font_size="1.1rem",
                    margin_bottom="2em",
                    margin_top="-1.5em",
                ...
                rx.hstack(
                    ...
                    rx.hstack(
                        rx.button(
                            ...
                            on_click=DatUpdateState.reset_form,
                            size="3",
                            style={
                        ...
                        rx.button(
                            rx.hstack(
                                ...
                                rx.cond(
                                    DatUpdateState.is_submitting,
                                    rx.text("Submitting..."),
                                    rx.text("Submit Update"),
                                ),
                                spacing="2",
                                align="center",
                            ),
                            on_click=DatUpdateState.submit_dat_update,
                            size="3",
                            disabled=rx.cond(
                                DatUpdateState.form_is_valid & ~DatUpdateState.is_submitting,
                                False,
                                True
                            ),
                            style={
                                "background": rx.cond(
                                    DatUpdateState.form_is_valid,
...
def Dats() -> rx.Component:
    """Main DAT updates page entry point."""
    return dats_with_custom_wrapper()
