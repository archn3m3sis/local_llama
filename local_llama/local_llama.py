...
from .pages import Dashboard, Dats, Images, Logs, Tickets, Assets, Playbook, PlaybookEditor, PlaybookEditorEnhanced, Software, Vulnerabilities, VMCreation, Analytics, ConfigurationManagement, Files
...
def configuration_management_with_custom_wrapper():
    """Configuration Management with custom wrapper to fix positioning."""
    return clerk.clerk_provider(
        clerk.clerk_loaded(
            clerk.signed_in(
                rx.fragment(
...
def files_with_custom_wrapper():
    """Files with custom wrapper to fix positioning."""
    return clerk.clerk_provider(
        clerk.clerk_loaded(
            clerk.signed_in(
                rx.fragment(
...
app.add_page(files_with_custom_wrapper, route="/files")
...
app.add_page(playbook_editor_with_custom_wrapper, route="/playbook/editor-old")

# Enhanced editor with WYSIWYG capabilities
def playbook_editor_enhanced_with_custom_wrapper():
    """PlaybookEditorEnhanced with custom wrapper."""
    return clerk.clerk_provider(
        clerk.clerk_loaded(
            clerk.signed_in(
                rx.fragment(
                    universal_background(),
                    PlaybookEditorEnhanced()
                )
            ),
            clerk.signed_out(
                page_wrapper(
...
# Use enhanced editor as the main editor
app.add_page(playbook_editor_enhanced_with_custom_wrapper, route="/playbook/editor")
